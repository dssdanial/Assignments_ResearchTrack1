...
while 1:
    if (silver == True) & (gold ==False): # if silver is True, than we look for a silver token, otherwise for a golden one
        ...
        if dist==-1: # if no token is detected, we make the robot turn 
	        ...
	        drive(-10,1)
	        turn(-20,3)
	        silver = False# we modify the value of the variable silver, so that in the next step we will look for the other type of token
        #else:
        ...
        print("dist from gold: "+str(dist) +"angle: "+str(rot_y))
        if (dist>obs_front):
                if (135<rot_y<180):
                    drive(30,2)
                    print("dist from gold: "+str(dist))
                else:
                    exit()


    """""    
        if dist <obs_front: # if we are close to the front obstacle, stop.
            print("Front obstacle!")
            turn(-20,3)
            gold = not gold # we modify the value of the variable gold, so that in the next step we will look for the other type of token
    elif (silver==False) & (gold==False):
        dist, rot_y = find_golden_token()
    elif (silver==True) & (gold==True):
        dist, rot_y = find_golden_token()
